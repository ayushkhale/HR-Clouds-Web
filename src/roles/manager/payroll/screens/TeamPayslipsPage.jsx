          ...
          <div className="p-6 overflow-y-auto space-y-6">
            ...
            {reimbursements.length > 0 && (
              <div>
                ...
                <div className="space-y-2 text-sm">
                  {reimbursements.map((r, i) => (<div key={i} className="flex justify-between"><span className="text-slate-600">{r.category || lineName(r) || "Reimbursement"}{r.claim_number ? ` · ${r.claim_number}` : ""}</span><span className="font-semibold text-purple-700 tabular-nums">{formatMoney(r.amount ?? r.calculated_amount)}</span></div>))}
                </div>
                {reimbTotal > 0 && <p className="text-[11px] text-slate-500 mt-2">Net pay includes {formatMoney(reimbTotal)} of reimbursements, which aren&apos;t part of gross pay.</p>}
              </div>
            )}
            {benefits.length > 0 && (
              <div>
                ...
                <div className="space-y-2 text-sm">
                  {benefits.map((b, i) => (<div key={i} className="flex justify-between"><span className="text-slate-600">{b.plan || b.plan_name || "Benefit"}</span><span className="text-slate-700 tabular-nums">You {formatMoney(b.employee ?? b.employee_contribution)} · Company {formatMoney(b.employer ?? b.employer_contribution)}</span></div>))}
                </div>
              </div>
            )}
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-5">
          {summary === undefined ? <Skeleton type="dashboard" /> : summary && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div className="bg-violet-50 rounded-xl p-3"><p className="text-[10px] font-bold text-violet-500 uppercase">Net</p><p className="text-base font-black text-violet-700 tabular-nums">{formatMoney(summary.total_net)}</p></div>
              <div className="bg-slate-50 rounded-xl p-3"><p className="text-[10px] font-bold text-slate-400 uppercase">LOP days</p><p className="text-base font-black text-slate-800 tabular-nums">{summary.total_lop_days ?? "N/A"}</p></div>
            </div>
          )}
            ...
            <div className="overflow-x-auto rounded-xl border border-slate-100">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-50">
                  {list.map((it, i) => (
                    <tr key={it.id || i}>
                      <td className="px-4 py-2.5 font-semibold text-slate-800">{it.employee_name || it.user?.profile?.display_name || it.user?.identifier || "N/A"}</td>
                      {!moneyHidden && <td className="px-4 py-2.5 text-right tabular-nums text-slate-700">{formatMoney(it.gross_earnings)}</td>}
                      {!moneyHidden && <td className="px-4 py-2.5 text-right tabular-nums font-bold text-violet-600">{formatMoney(it.net_pay)}</td>}
                      <td className="px-4 py-2.5 capitalize text-slate-500">{it.status || "N/A"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
