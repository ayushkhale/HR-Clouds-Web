...
const TYPE_COLORS = {
  ...
  rotational: "bg-emerald-50 text-emerald-700",
};
...
function ShiftModal({ onClose, onSaved }) {
  const [form, setForm] = useState({
    ...
    core_end_time: "15:00",
    split_start_time_2: "14:00",
    split_end_time_2: "19:00",
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div>
            <h2 className="text-base font-bold text-slate-800">Add Shift Template</h2>
            <p className="text-xs text-slate-400 mt-0.5">Define working hours for a shift.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          ...
          {/* Shift Type Pills */}
          <div>
            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Shift Type</label>
            <div className="grid grid-cols-2 gap-2">
              {SHIFT_TYPES.map((t) => (
                ...
                >
                  <span className={`text-xs font-bold ${form.type === t.value ? "text-purple-700" : "text-slate-700"}`}>{t.label}</span>
          ...
          {(isFixed || isSplit) && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Shift Start Time</label>
                <input type="time" value={form.start_time} onChange={(e) => set("start_time", e.target.value)}
                  className="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Shift End Time</label>
          ...
          {isSplit && (
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Second Session</label>
              <div className="grid grid-cols-2 gap-4">
                <div>
          ...
          {isFlexible && (
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Minimum Working Hours</label>
                <input type="number" step={0.5} min={1} value={form.min_hours} onChange={(e) => set("min_hours", e.target.value)}
                  className="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition" />
                <p className="text-[10px] text-slate-400 mt-1">Employee must work at least this many hours</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Core Hours Start</label>
                  <input type="time" value={form.core_start_time} onChange={(e) => set("core_start_time", e.target.value)}
                    className="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Core Hours End</label>
                  <input type="time" value={form.core_end_time} onChange={(e) => set("core_end_time", e.target.value)}
                    className="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition" />
                </div>
              </div>
              <p className="text-[10px] text-slate-400">Employees must be present during core hours window.</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Early Clock-in Buffer (mins)</label>
              <input type="number" min={0} value={form.buffer_minutes_before} onChange={(e) => set("buffer_minutes_before", e.target.value)}
                className="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-100 transition" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Late Clock-in Buffer (mins)</label>
...
export default function AttendanceShiftsPage() {
  ...
  async function handleDelete(shift) {
    ...
    try {
      ...
      if (err.status === 400) {
        showToast("This shift is assigned to employees and cannot be deleted. Deactivate it instead.", "error");
      } else {
