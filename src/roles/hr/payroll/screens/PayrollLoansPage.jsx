    ...
    <>
        ...
        <main className="flex-1 overflow-y-auto p-6 sm:p-8 max-w-7xl mx-auto w-full">
          ...
          <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Loans &amp; Advances
              </h1>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-1.5">
                          {["active", "closed", "foreclosed"].includes(l.status) && (
                            <button onClick={() => openSchedule(l)} className="p-1.5 text-slate-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition" title="Repayment schedule"><HiCalendar className="w-4 h-4" /></button>
                          )}
                          {l.status === "pending" && (
                            <>
                          ...
                          {l.status === "active" && (
                            <button onClick={() => { setFcForm({ settlement_mode: "recover_via_payroll", recovery_month: now.getMonth() + 1, recovery_year: now.getFullYear(), reason: "" }); setForeclosing(l); }} className="px-2.5 py-1.5 text-[11px] font-bold text-slate-600 bg-slate-50 hover:bg-slate-100 hover:text-purple-700 rounded-lg transition flex items-center gap-1"><HiLightningBolt className="w-3.5 h-3.5" /> Foreclose</button>
                          )}
                          {(l.status === "pending" || l.status === "active") && (
                            <button onClick={() => window.confirm("Cancel this loan? Only allowed with zero deducted installments.") && act(() => payrollAPI.cancelLoan(l.id), "Loan cancelled")} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition" title="Cancel"><HiTrash className="w-4 h-4" /></button>
                          )}
                        </div>
                      </td>
                    </tr>
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  ...
                  <span className={`ml-2 px-1.5 py-0.5 rounded-md border text-[9px] font-bold uppercase tracking-wider ${STATUS_PILL[schedule.loan.status] || "bg-slate-50 text-slate-600 border-slate-200"}`}>{schedule.loan.status}</span>
                </p>
              </div>
                  ...
                  {schedule.installments.map((inst) => (
                    <tr key={inst.id || inst.installment_number} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-5 py-2.5 text-center">
                        <span className={`px-2 py-0.5 rounded-md border text-[9px] font-bold uppercase tracking-wider ${INST_PILL[inst.status] || "bg-slate-50 text-slate-500 border-slate-200"}`}>{inst.status}</span>
                      </td>
                    </tr>
                  ))}
