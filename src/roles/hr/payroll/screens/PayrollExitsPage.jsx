  ...
  return (
    <>
      <DetailDialog
        ...
        icon={HiCash}
        title={nameOf(exit.user_id, "Final settlement")}
        subtitle={`${exitTypeLabel(exit.exit_type)} · last day ${formatDate(exit.last_working_day)}`}
        badge={<StatusPill status={exit.status} />}
        onClose={onClose}
        footer={
          <>
            {!acts.canPrepare && !acts.canReset && (
              <p className="mr-auto flex items-start gap-2 text-xs text-slate-500">
                <HiInformationCircle className="w-4 h-4 shrink-0 text-purple-500" /> <span>{acts.prepareReason}</span>
              </p>
            )}
            ...
            {acts.canPrepare && (
              <button type="button" onClick={prepare} disabled={busy || loading || !!error}
                className="px-4 py-2.5 rounded-xl font-bold text-sm bg-purple-600 text-white hover:bg-purple-700 transition shadow-md shadow-purple-200 disabled:opacity-50">
      ...
      >
          ...
          <div className="sm:max-w-xs">
            ...
            {prepared && (
              <p className="flex items-start gap-1.5 text-[10px] text-slate-500 mt-1.5">
                <HiInformationCircle className="w-3.5 h-3.5 shrink-0 mt-px text-purple-500" />
                Fixed while the settlement is locked in. Undo it to change the month.
              </p>
            )}
          </div>
        </DetailSection>
        ...
        <DetailSection
          title="What they are owed"
          icon={HiCash}
          collapsible={false}
          action={prepared ? <DetailPill tone="solid">Locked in</DetailPill> : <DetailPill tone="muted">Not locked in yet</DetailPill>}
        >
