    ...
    <>
        ...
        <main className="flex-1 overflow-y-auto p-6 sm:p-8 max-w-7xl mx-auto w-full">
          ...
          <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Bonus Rules
              </h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={openCreate} className="h-[42px] px-4 text-sm font-bold bg-purple-600 text-white hover:bg-purple-700 rounded-xl transition flex items-center gap-2 shadow-md shadow-purple-200">
                  ...
                  {rules.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        {r.applied_at && (
                          <span className="inline-block mt-1.5 text-[9px] font-bold text-slate-500 border border-slate-200 px-1.5 py-0.5 rounded">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-md border text-[10px] font-bold uppercase tracking-wider ${STATUS_PILL[r.status] || "bg-slate-50 text-slate-600 border-slate-200"}`}>
                          {r.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-1.5">
                          <button onClick={() => openDetail(r)} className="p-1.5 text-slate-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition" title="View details"><HiEye className="w-4 h-4" /></button>
                          {(r.status === "pending" || r.status === "approved") && !r.applied_at && (
                            <button onClick={() => openImpact(r)} className="p-1.5 text-slate-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition" title="Preview impact"><HiCalculator className="w-4 h-4" /></button>
                          )}
                          {r.status === "pending" && (
                            <>
                              <button onClick={() => openEdit(r)} className="p-1.5 text-slate-400 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition" title="Edit"><HiPencil className="w-4 h-4" /></button>
                              <button onClick={() => act(() => payrollAPI.approveBonusRule(r.id), "Bonus rule approved")} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition" title="Approve"><HiCheck className="w-4 h-4" /></button>
                              <button onClick={() => setRejectingId(r.id)} className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition" title="Reject"><HiX className="w-4 h-4" /></button>
                            </>
                          )}
                          ...
                          {(r.status === "pending" || r.status === "approved") && !r.applied_at && (
                            <button onClick={() => window.confirm("Cancel this bonus rule?") && act(() => payrollAPI.cancelBonusRule(r.id), "Bonus rule cancelled")} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition" title="Cancel"><HiTrash className="w-4 h-4" /></button>
                          )}
                        </div>
                      </td>
                    </tr>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95">
              ...
              <div className="p-6 overflow-y-auto space-y-5">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2 py-1 rounded-md border text-[10px] font-bold uppercase tracking-wider ${STATUS_PILL[detail.status] || "bg-slate-50 text-slate-600 border-slate-200"}`}>{detail.status}</span>
                </div>
