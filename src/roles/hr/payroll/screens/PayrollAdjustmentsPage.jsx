...
import { HiCheckCircle, HiExclamationCircle, HiX, HiPlus, HiUpload, HiCheck, HiAdjustments, HiTrash, HiEye } from "react-icons/hi";
...
export default function PayrollAdjustmentsPage() {
  ...
  const [detail, setDetail] = useState(null);        // full adjustment (#59)
  ...
  const openDetail = async (adj) => {
    setDetail({ __row: adj });   // show the drawer immediately with what we know
    setDetailLoading(true);
    try {
      const res = await payrollAPI.getAdjustment(adj.id);
      setDetail(res.data || adj);
    } catch (err) {
      showToast(err.message || "Failed to load adjustment", "error");
      setDetail(null);
    } finally {
  ...
  return (
    <>
      ...
      {/* Adjustment detail drawer (#59) */}
      {detail && (() => {
        const d = detail.__row || detail;
        const empName = employees.find(e => (e.id || e.user_id || e._id) === d.user_id)?.name || d.employee?.name || d.user_id;
        const isEarning = d.adjustment_type === "earning";
        const flag = (on) => on
          ? <span className="text-emerald-600 font-bold">Yes</span>
          : <span className="text-slate-400">No</span>;
        const meta = [
          ["Created by", d.created_by_name || d.created_by],
          ["Created on", d.created_at ? formatDate(d.created_at) : null],
          ["Approved by", d.approved_by_name || d.approved_by],
          ["Approved on", d.approved_at ? formatDate(d.approved_at) : null],
          ["Rejection reason", d.rejection_reason],
          ["Batch", d.batch_id ? "Part of a bulk batch" : null],
          ["Bonus rule", d.bonus_rule_id ? "Generated by a bonus rule" : null],
          ["Applied run", d.applied_run_id ? "Applied to a payroll run" : null],
        ].filter(([, v]) => v != null && v !== "");
        return (
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95">
              <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                <div>
                  ...
                  <p className="text-xs text-slate-500">{empName} · {formatPeriod(d.period_month)}</p>
                </div>
              ...
              <div className="p-6 overflow-y-auto space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{isEarning ? "Earning" : "Deduction"} · {(d.category || "").replace(/_/g, " ")}</p>
                    <p className={`text-2xl font-black mt-0.5 ${isEarning ? "text-emerald-600" : "text-red-600"}`}>{isEarning ? "+" : "−"}{formatMoney(d.amount)}</p>
                  </div>
                  <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${(d.status === 'pending' || d.status === 'proposed') ? 'bg-amber-100 text-amber-700' : d.status === 'approved' ? 'bg-emerald-100 text-emerald-700' : d.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700'}`}>{d.status}</span>
                </div>

                {(d.component_name || d.reason) && (
                  <div className="rounded-xl border border-slate-100 divide-y divide-slate-50">
                    {d.component_name && <div className="flex items-center justify-between px-4 py-3"><span className="text-[11px] font-bold text-slate-400 uppercase">Component</span><span className="text-sm font-semibold text-slate-800">{d.component_name}</span></div>}
                    {d.reason && <div className="px-4 py-3"><p className="text-[11px] font-bold text-slate-400 uppercase mb-1">Reason</p><p className="text-sm text-slate-700">{d.reason}</p></div>}
                  </div>
                )}

                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="bg-slate-50 rounded-xl py-3"><p className="text-[10px] font-bold text-slate-400 uppercase">Taxable</p><p className="text-sm mt-1">{flag(d.is_taxable)}</p></div>
                  <div className="bg-slate-50 rounded-xl py-3"><p className="text-[10px] font-bold text-slate-400 uppercase">PF</p><p className="text-sm mt-1">{flag(d.pf_applicable)}</p></div>
                  <div className="bg-slate-50 rounded-xl py-3"><p className="text-[10px] font-bold text-slate-400 uppercase">ESI</p><p className="text-sm mt-1">{flag(d.esi_applicable)}</p></div>
                </div>

                {detailLoading ? (
                  <p className="text-xs text-slate-400 text-center">Loading full audit trail…</p>
                ) : meta.length > 0 && (
